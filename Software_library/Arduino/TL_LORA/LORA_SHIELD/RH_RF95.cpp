...
void RH_RF95::handleInterrupt()
{
    ...
    // if (_mode == RHModeRx && irq_flags & (RH_RF95_RX_TIMEOUT)) // [@]cgl, here we remove the CRC check handler.
    if (_mode == RHModeRx && irq_flags & (RH_RF95_RX_TIMEOUT | RH_RF95_PAYLOAD_CRC_ERROR))
    {
	_rxBad++;
    }
...
{
	// Serial.println("[!1] test if receive.");
    if (!available())
	return false;
    if (buf && len)
    {
    ...
    // Serial.println("[!2] test return true");
...
void RH_RF95::setSpreadingFactor(int8_t sf)
{
  ...
  // Serial.print("spiRead(RH_RF95_REG_1E_MODEM_CONFIG2): " );
  // Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2), BIN);
  // Serial.print("spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & 0x0f): " );
  // Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & 0x0f, BIN);
  // Serial.print("(sf << 4): " );
  // Serial.println(sf << 4, BIN);
  // Serial.print("(sf << 4) & 0xf0: " );
  // Serial.println((sf << 4) & 0xf0, BIN);

  spiWrite(RH_RF95_REG_1E_MODEM_CONFIG2, (spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & 0x0f) | ((sf << 4) & 0xf0));

  // Serial.print("[after set] spiRead(RH_RF95_REG_1E_MODEM_CONFIG2): " );
  // Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2), BIN);
...
// =================[@]cgl add.
// [@]CGL, currently, we don't know whether it works.
void RH_RF95::setCRCState(int8_t crcState)
{
  Serial.print("crc state berfore:");
  Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2+RH_RF95_RX_PAYLOAD_CRC_ON-1), BIN);
      Serial.print("spiRead(RH_RF95_REG_1E_MODEM_CONFIG2):");
  Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2), BIN);
            Serial.print("spiRead(RH_RF95_REG_1C_HOP_CHANNEL):");
  Serial.println(spiRead(RH_RF95_REG_1C_HOP_CHANNEL), BIN);

	spiWrite(RH_RF95_REG_1E_MODEM_CONFIG2, (spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & 0xfb) | ((crcState << 2) & 0x04));

	spiWrite(RH_RF95_REG_1C_HOP_CHANNEL, (spiRead(RH_RF95_REG_1C_HOP_CHANNEL) & 0xbf) | ((crcState << 6) & 0x40));


    Serial.print("crc state after:");
  Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2+RH_RF95_RX_PAYLOAD_CRC_ON-1), BIN);
        Serial.print("spiRead(RH_RF95_REG_1E_MODEM_CONFIG2):");
  Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2), BIN);
          Serial.print("spiRead(RH_RF95_REG_1C_HOP_CHANNEL):");
  Serial.println(spiRead(RH_RF95_REG_1C_HOP_CHANNEL), BIN);
	// spiWrite(RH_RF95_RX_PAYLOAD_CRC_ON, crcState);
}

// [@]cgl add.
int16_t RH_RF95::getCurrentRssi()
{
    int16_t currentRssi;

    // currentRssi = spiRead(RH_RF95_REG_1A_PKT_RSSI_VALUE) - 137;

    currentRssi = spiRead(RH_RF95_REG_1B_RSSI_VALUE) - 137;
    // currentRssi = spiRead(RH_RF95_REG_1B_RSSI_VALUE) - 164;
    return currentRssi;
}

int16_t RH_RF95::getLastSNR()
{
    return _lastSNR;
}
...
bool RH_RF95::isRx()
{
    ...
    // Serial.print("spiRead(RH_RF95_REG_1C_HOP_CHANNEL):");
    // Serial.println(spiRead(RH_RF95_REG_1C_HOP_CHANNEL), BIN);
    // onrxstate = spiRead(RH_RF95_REG_18_MODEM_STAT) >> 2 & 0x01;
    // if (onrxstate)
    //     return true;
    ...
    // int8_t CadDone;
    // int8_t RxDone;
    // CadDetected = spiRead(RH_RF95_REG_12_IRQ_FLAGS) & 0x01;
    // CadDone = (spiRead(RH_RF95_REG_12_IRQ_FLAGS) >> 2) & 0x01;
    // if(CadDetected == 1)
    //     return true;


    // uint8_t irq_flags = spiRead(RH_RF95_REG_12_IRQ_FLAGS);
    // if( irq_flags & RH_RF95_CAD_DETECTED )
    //     return true;
...
bool RH_RF95::isRxDone()
{
    ...
    // int8_t RxDone;
    // RxDone = (spiRead(RH_RF95_REG_12_IRQ_FLAGS) >> 6) & 0x01;
    // if (RxDone == 1)
    //     return true;
    return false;
}

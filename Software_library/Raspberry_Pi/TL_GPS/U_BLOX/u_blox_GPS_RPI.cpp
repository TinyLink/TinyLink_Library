...
namespace tinylink {
	...
	static Serial U_BLOX_GPS_Serial;	

	u_blox_GPS::u_blox_GPS():Sensor({2017, 3, 29, 0, 0, 0, 0, 0, 'N', 'E'}){
		U_BLOX_GPS_Serial.begin(9600);
	}	
	

	int u_blox_GPS::getComma(unsigned char num, char* str){
		unsigned char i, j = 0;
	...
	double u_blox_GPS::getDoubleNumber(char *s){
		char buf[20];
		memset(buf, 0, 20);
		unsigned char i;
		double rev;
	...
	int u_blox_GPS::parse(char *gpsData){
		double latitude = 0;
		double longitude = 0;
		int tmp, checksum;
		if(gpsData[0] == '$')
